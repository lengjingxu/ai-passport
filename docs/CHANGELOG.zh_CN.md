...
- 任务列表改为两行标题，移除摘要；状态固定在第二行右侧，并为其预留空间。

- 任务卡片为名称预留两行实际字高，状态和摘要分行排列；详情与录音标题限制高度，避免长名称与相邻文案重叠。

- 开机直接进入 Cindy，显示连接引导和任务位置；长按确认取消未确认录音并返回首页，保持蓝牙连接。应用构建移除硬件测试入口。

- Passport 显示任务状态、分页的 Cindy 回复和转写预览；上键重录，下键阅读，确认键发送到原任务，语音识别跟随 Cindy 当前配置。

- 根据中文任务页与录音页的内存测量，将 LVGL 池调整为 32 KB，为 ESP32-C3 上的 Opus 初始化留出连续堆内存。

- 链路加密失败或超时时保留已保存的配对记录，只在链路已加密但未认证（just-works）时删除，避免一次重连失败就让 Mac 保留设备已丢弃的配对信息；同时记录蓝牙连接、断开与安全状态。

- 退出任务页后保持蓝牙链路和最后一次收到的任务列表，重新进入直接显示上次任务并继续接收新快照，不再丢配对或清空列表。

- 蓝牙语音反馈改用 40 ms Opus 帧和可变载荷长度，降低 ESP32-C3 录音栈峰值；客户端按完整 Ogg 时间轴和有界包数接收。

- 使用驻留 Flash 的 Noto 中文字库显示任务文字，截断文本时保留 UTF-8 字符边界；为八张任务卡片调大 LVGL 内存池；让有界 Opus BLE 录音在 ESP32-C3 堆内存预算内启动，并通过已认证的蓝牙 indication 发送给 Cindy 转写适配器。已完成任务由客户端保留。

- 新增 Cindy BLE 任务模式：六位码认证配对、有界完整快照、等待交互状态颜色、过期状态清空，以及按任务 ID 打开电脑任务。需配套 macOS Cindy 适配器。

- 调整 Cindy 任务卡片与详情，分开标题、状态和限高滚动消息；固定显示按键提示与 bridge 文件来源，录音页显示目标任务。保留像素主题与现有按键操作。

- 录音期间关闭 Wi-Fi 省电，结束时恢复原模式；PCM 上传连接关闭 TCP 小包合并，并记录已发送字节数和最长写入耗时。

- 录音队列加入最多 20 ms 的等待，让上传任务处理麦克风缓存突发，修复立即判定队列超时的问题；增加录音控制流程回归测试，以及任务来源、上传中断日志。

- 录音改为固定队列分块上传，上限 30 秒，移除整段录音内存分配；拒绝保存未完成录音，增加内存和传输错误日志。固件须与新版本地桥接服务一起使用。
