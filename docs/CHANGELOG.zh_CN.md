...
- 链路不是认证加密时删除已保存的配对记录，并记录蓝牙连接、断开与安全状态；Mac 残留旧密钥时能够重新配对，不再每次重连都被断开。

- 使用驻留 Flash 的 Noto 中文字库显示任务文字，截断文本时保留 UTF-8 字符边界；增加有界 Opus 录音与经过认证的蓝牙 indication 传输，配合 Cindy 客户端转写。已完成任务由客户端保留。
