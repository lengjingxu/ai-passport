...
UP/DOWN selects a task; OK opens its details. In details, UP/DOWN changes tasks and OK starts recording. Another OK stops and sends; 30 seconds submits automatically. Holding OK exits and cancels. The microphone and sender have separate workers and an eight-block PCM queue. A full queue waits 20 ms, then aborts. Each indication waits at most 1.5 seconds for acknowledgement. Codec processing uses a 24 KB sender stack; actual heap and timing require board measurement.
