...
使用配套 Cindy macOS 客户端，在设置 → 快捷键 → 配件 → Cindy Passport 中启用。设备开机直接进入 Cindy，电脑选择发现的设备标识；macOS 请求配对时输入设备屏幕显示的码。界面显示权限与连接状态，不需要 IP 或 HTTP 桥接。菜单栏控制仍可使用。

任务列表显示状态，上下键选择，确认键进入详情。详情中上下键翻页阅读 Cindy 的最新可见回复，确认键开始录音，再按确认停止并转写；30 秒自动停止。设备随后显示转写文字：上键丢弃并重录，下键循环翻页，确认键发送到原任务。长按确认返回任务首页并取消未确认录音，蓝牙保持连接；已经确认的发送完成前不能返回。回复读取遵守清空与回溯边界。
...
任务控制 TX 通知为 `version=2:u8, action:u8, taskId:40 bytes, recordingToken:u32LE`（46 字节，ATT MTU 至少 49）。动作 1–6 分别为进入、上一页、下一页、重录、确认和取消。预览操作必须匹配任务和录音标识。快照状态 `draft:<8位十六进制标识>` 表示待确认，`retry:<标识>` 表示发送被拒，`asr:<标识>` 表示正在转写，`error:<标识>` 表示转写失败；设备显示中文状态，不显示标识。文字在 192 字节 message 字段内以 `[页码/总页数]` 和四行短文本分页。固件与电脑端须一起更新。
...
源码保留旧局域网开发入口，但 Cindy 应用不提供此入口。该模式使用忽略提交的 `main/app_config.h` 和 `python3 bridge/server.py --port 8787`。任务来源是文件，HTTP 分块 PCM 保存为 WAV，不提供真实 Cindy 事件或转写。蓝牙不会切换到这条路径。本地配置固件可能含凭证，不能公开上传。
...
开机直接进入 Cindy 任务首页，保留像素天空、草地与吉祥物。空状态区分连接引导、已连接无任务和心跳暂停。页脚显示任务位置与当前操作提示，不显示传输实现细节。长按确认取消未确认录音并返回任务首页，蓝牙保持连接；已经确认的发送完成前不能返回。硬件初始化失败显示错误并停止启动，不切换传输方式或测试菜单。旧 LAN 模式仅保留开发源码入口，不在 Cindy 应用中提供入口。

官方 `demo/claude-buddy-port` 提供直接启动应用与持续连接状态的参考；`demo/stopwatch` 和 `demo/cat-themed-pomodoro-timer` 提供纯逻辑主机测试与单调计时模式；`demo/tetris-game` 提供输入入队和按变化更新列表的模式。复用程序绘制素材，避免 `demo/rock-paper-scissors` 中全屏 RGB565 素材的 Flash 成本。
