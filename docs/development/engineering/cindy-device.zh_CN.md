...
完整录音在 Mac 内存中解码为 16 kHz 单声道 PCM，交给 Cindy 当前选择的语音输入服务和识别模型。Cindy 内置语音使用已有会话服务，用户显式配置的供应商沿用已有凭据与连接方式。转写期间切换语音配置或账号会中止该段录音，Passport 不自行改选模型。转写后打开原任务，只填入空白草稿；已有草稿保留，新文字等待草稿为空。用户在 Cindy 检查并发送，不自动发送消息，也不执行设备审批。取消、乱序、截断、超限、断连的录音都会拒绝，转写前后复核任务目录、账号及连接边界。
